...
parser = argparse.ArgumentParser(description='photon fake rate')
...
#         chg_cut = (df.loc[:,'photon_pfRelIso03_chg']*df.loc[:,'photon_pt']) < 1.051
    ...
    df['mediumID'] = df['photon_vidNestedWPBitmap'] & mask_mediumID_withoutsieie
    df = df.loc[df.loc[:,'mediumID'] == mask_mediumID_withoutsieie, :]
    
    pt_cut = (df.loc[:,'photon_pt'] >= ptrange[0]) & (df.loc[:,'photon_pt'] < ptrange[1]) 
    df = df.loc[pt_cut & eta_cut,:]
    
    for i in trange(0, len(df['photon_sieie']), desc=f'fill sigma ieta ieta for {file}'):
        hist.Fill(float(df['photon_sieie'].values[i]))
    return hist
...
def AddHist_mcTruth(file, hist, ptrange, isbarrel, xsec, lumi, year='2018', **kwargs):
    ...
    df['true_weight'] = lumi * xsec * 1000 * df['Generator_weight_sgn'] / true_events
    
    for i in trange(0, len(df['photon_sieie']), desc=f'fill sigma ieta ieta for {file}'):
        hist.Fill(float(df['photon_sieie'].values[i]), float(df['true_weight'].values[i]))
    return hist
...
def AddHist_dataFake(file, hist, ptrange, isbarrel, year='2018', **kwargs):
    ...
    mask_mediumID_without_chgisosieie = (1<<1) | (1<<3) | (1<<5) | (1<<11) | (1<<13)
    df['mediumID'] = df['photon_vidNestedWPBitmap'] & mask_mediumID_without_chgisosieie
    df = df.loc[df.loc[:,'mediumID'] == mask_mediumID_without_chgisosieie, :]

    chg_cut = ((df.loc[:,"photon_pfRelIso03_chg"]*df.loc[:,"photon_pt"]) > 4) & ((df.loc[:,"photon_pfRelIso03_chg"]*df.loc[:,"photon_pt"]) < 10)
    pt_cut = (df.loc[:,'photon_pt'] >= ptrange[0]) & (df.loc[:,'photon_pt'] < ptrange[1]) 
    df = df.loc[pt_cut & eta_cut & chg_cut ,:]
    
    for i in trange(0, len(df['photon_sieie']), desc=f'fill sigma ieta ieta for {file}'):
        hist.Fill(float(df['photon_sieie'].values[i]))
    return hist
        
def AddHist_mcTruth_infake(file, hist, ptrange, isbarrel, xsec, lumi, year='2018', **kwargs):
    init_branches = []
    init_branches = AddBranches(file, year, init_branches)
    init_branches.extend(['Generator_weight','photon_genPartFlav'])
    true_events = uproot.open(file)['nEventsGenWeighted'].values()[0]
    ...
    pt_cut = (df.loc[:,'photon_pt'] >= ptrange[0]) & (df.loc[:,'photon_pt'] < ptrange[1]) 
    gen_cut = df.loc[:,'photon_genPartFlav'] == 1
    df = df.loc[pt_cut & eta_cut & gen_cut & chg_cut,:]
...
if __name__ == '__main__':
    ...
    if isbarrel:
        ...
        xleft = 0.00615
        xright = 0.02015
        ptlist = [15, 20, 30, 50, 70, 100, 5000]
    else:
        xbins = 8
        xleft = 0.0172
        xright = 0.0572
    ...
    for i, ptlow in enumerate(ptlist[:len(ptlist)-1]):
        ...
        for file in filelist_data:
            if str(args.year) == '2016':
                if '2016Pre' in file:
                    AddHist_data(file, hist_data, ptrange, isbarrel, year='2016Pre')
                elif '2016Post' in file:
                    AddHist_data(file, hist_data, ptrange, isbarrel, year='2016Post')
            else:
                AddHist_data(file, hist_data, ptrange, isbarrel, year=str(args.year))

        for file in filelist_mc:
            if str(args.year) == '2016':
                if '2016Pre' in file:
                    AddHist_mcTruth(file, hist_mctruth, ptrange, isbarrel, xsec, 19.5, year='2016Pre')
                elif '2016Post' in file:
                    AddHist_mcTruth(file, hist_mctruth, ptrange, isbarrel, xsec, 16.8, year='2016Post')
            elif str(args.year) == '2018':
                AddHist_mcTruth(file, hist_mctruth, ptrange, isbarrel, xsec, 59.7, year=str(args.year))
            elif str(args.year) == '2017':
                AddHist_mcTruth(file, hist_mctruth, ptrange, isbarrel, xsec, 41.5, year=str(args.year))

        for file in filelist_data:
            if str(args.year) == '2016':
                if '2016Pre' in file:
                    AddHist_dataFake(file, hist_datafake, ptrange, isbarrel, year='2016Pre')
                elif '2016Post' in file:
                    AddHist_dataFake(file, hist_datafake, ptrange, isbarrel, year='2016Post')

            elif str(args.year) == '2018':
                AddHist_dataFake(file, hist_datafake, ptrange, isbarrel, year=str(args.year))
            elif str(args.year) == '2017':
                AddHist_dataFake(file, hist_datafake, ptrange, isbarrel, year=str(args.year))
        ...
        for file in filelist_mc:
            if str(args.year) == '2016':
                if '2016Pre' in file:
                    AddHist_mcTruth_infake(file, hist_datafake, ptrange, isbarrel, xsec, 19.5, year='2016Pre')
                elif '2016Post' in file:
                    AddHist_mcTruth_infake(file, hist_datafake, ptrange, isbarrel, xsec, 16.8, year='2016Post')
            elif str(args.year) == '2018':
                AddHist_mcTruth_infake(file, hist_datafake, ptrange, isbarrel, xsec, 59.7, year=str(args.year))
            elif str(args.year) == '2017':
                AddHist_mcTruth_infake(file, hist_datafake, ptrange, isbarrel, xsec, 41.5, year=str(args.year))
        ...
        if isbarrel == 1:
            sieie.setRange('window', 0.00615, 0.01015)
        else:
        ...
        c1.SaveAs(f'{args.year}/{region_mark}_pt{str(ptrange[0])}_{str(ptrange[1])}_v9.pdf')
        c1.SaveAs(f'{args.year}/{region_mark}_pt{str(ptrange[0])}_{str(ptrange[1])}_v9.png')
        # print (ntrue.getVal())
