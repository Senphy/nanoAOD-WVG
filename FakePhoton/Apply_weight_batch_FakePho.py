...
parser.add_argument('-p', dest='path', default='/eos/user/s/sdeng/WZG_analysis/fake_photon_template/AR/', help='File path input')
...
if __name__ == '__main__':
    ...
    for file in os.listdir(store_path):
        if file.endswith('.root'):
            try:
                os.system('python Apply_weight_FakePho_Template_postproc.py -f {store_path}/{file} -y {year}'.format(store_path=store_path, file=file, year=args.year))
            except:
