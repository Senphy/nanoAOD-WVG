...
def prepare_crab(name,sample_type,year,period):
    ...
    with open('crabcode_' + year + '/' + abbre_name + '_cfg.py', 'w+') as f:
        ...
        f.write('config.JobType.scriptExe = "./test_crab.sh" \n')
        f.write('config.JobType.inputFiles = ["../../scripts/haddnano.py","./ntuple_Template_Module.py","./ntuple_Template_postproc.py","./keep_and_drop.txt","./output_branch_selection.txt","../WZG_selector/DAS_filesearch.py","/afs/cern.ch/cms/CAF/CMSCOMM/COMM_DQM/certification/Collisions18/13TeV/Legacy_2018/Cert_314472-325175_13TeV_Legacy2018_Collisions18_JSON.txt"] #hadd nano will not be needed once nano tools are in cmssw \n')
        ...
        f.write('config.Data.outLFNDirBase ="/store/user/sdeng/WZG_analysis/test/' + sample_type + '/' + year + '"\n')
...
def hadd_help(name,sample_type,year):
    ...
    abbre_name = get_abbre(name,sample_type,year)
    store_path = '/eos/user/s/sdeng/WZG_analysis/test/'
