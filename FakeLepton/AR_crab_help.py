...
def prepare_crab(name,sample_type,year):
    ...
    if not os.path.exists('AR_crabcode_' + year):
        os.mkdir("AR_crabcode_" + year)

    print ("------> preparing submit code for",abbre_name)
    with open('AR_crabcode_' + year + '/' + abbre_name + '_cfg.py', 'w+') as f:
        ...
        f.write('config.General.workArea = "AR_crab' + year + '"\n\n')

        f.write('config.section_("JobType")\n')
        f.write('config.JobType.pluginName = "Analysis"\n')
        f.write('config.JobType.psetName = "PSet.py"\n')
        f.write('config.JobType.scriptExe = "./AR_crab_script.sh" \n')
        f.write('config.JobType.inputFiles = ["../../scripts/haddnano.py","./AR_Template_postproc.py","./AR_Template_Module.py","./AR_keep_and_drop.txt","./AR_output_branch_selection.txt"] #hadd nano will not be needed once nano tools are in cmssw \n')
        ...
        f.write('config.Data.outLFNDirBase ="/store/user/sdeng/WZG_analysis/fake_lepton_template/AR/' + sample_type + '/' + year + '"\n')
...
def submit(name,sample_type,year):
    ...
    if not os.path.exists(f'AR_crabcode_{year}/{abbre_name}_cfg.py'):
        print ("AR_crabcode for ",abbre_name," not existed, \033[31mskipping\033[0m")
        return True

    r=subprocess.run(args=f"crab submit -c AR_crabcode_{year}/{abbre_name}_cfg.py",shell=True,stdout=subprocess.PIPE,encoding='utf-8')
...
def kill(name,sample_type,year):
    ...
    if not os.path.exists(f'AR_crab{year}/crab_{abbre_name}'):
        print ("crab log for ",abbre_name," not existed, \033[31mskipping\033[0m \n")
        return True

    r=subprocess.run(args=f"crab kill -d AR_crab{year}/crab_{abbre_name}" ,shell=True,stdout=subprocess.PIPE,encoding='utf-8')
    print (r.stdout,'\n')

    shutil.rmtree(f'AR_crab{year}/crab_{abbre_name}')
    print (f'AR_crab{year}/crab_{abbre_name} has been removed')
...
def status(name,sample_type,year):
    ...
    r=subprocess.run(args=f"crab status -d AR_crab{year}/crab_{abbre_name}" ,shell=True,stdout=subprocess.PIPE,encoding='utf-8')
    print (r.stdout,'\n')
...
def report_lumi(name,sample_type,year):
    ...
    r=subprocess.run(args=f"crab report -d AR_crab{year}/crab_{abbre_name}" ,shell=True,stdout=subprocess.PIPE,encoding='utf-8')
    print (r.stdout,'\n')

    if not os.path.exists(f'AR_lumi_{year}'):
        os.mkdir(f'AR_lumi_{year}')
    
    shutil.copy(f'AR_crab{year}/crab_{abbre_name}/results/notFinishedLumis.json', f'AR_lumi_{year}/{abbre_name}.json')
...
def resubmit(name,sample_type,year):
    ...
    r = subprocess.run(args=f"crab resubmit -d AR_crab{year}/crab_{abbre_name}" ,shell=True,stdout=subprocess.PIPE,encoding='utf-8')
    print (r.stdout,"\n")
