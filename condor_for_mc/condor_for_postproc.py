...
for dataset in jsons:
    ...
    datasetname = dataset['name'].split('/')[1].split('_')[0]+'_'+dataset['year'] # +"_"+dataset['name'].split('/')[2]
    ...
    for line in lines:
        line = line.rstrip('\n')
        filepath = line
        # valid site search has been moved in postproc process
        # filepath = search.getValidSite(line)+line 
        # filepath = "root://cms-xrd-global.cern.ch/"+line 
        ...
        with open ("wrapper_"+datasetname+"_file"+str(i)+"_"+filename+".sh","w+") as f:
            ...
            f.write("scramv1 project CMSSW CMSSW_10_6_19\n")
            f.write("cd CMSSW_10_6_19/src\n")
            ...
            f.write("git clone https://github.com/Senphy/nanoAOD-WVG.git \n")
            f.write("cd nanoAOD-WVG/WZG_selector\n")
            # f.write("cp ${initial_path}/filepath_"+datasetname+".txt .\n" )
            # f.write("python WZG_postproc_data.py -f "+filepath+" -j "+dataset['json']+"\n\n")
            f.write("python WZG_postproc.py -f "+filepath+" -m condor\n\n")
