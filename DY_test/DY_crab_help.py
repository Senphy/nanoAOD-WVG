...
def prepare_crab(name,sample_type,year):
    ...
    if not os.path.exists('DY_crabcode_' + year):
        os.mkdir("DY_crabcode_" + year)

    print ("------> preparing submit code for",abbre_name)
    with open('DY_crabcode_' + year + '/' + abbre_name + '_cfg.py', 'w+') as f:
        ...
        f.write('config.General.workArea = "DY_crab' + year + '"\n\n')

        f.write('config.section_("JobType")\n')
        f.write('config.JobType.pluginName = "Analysis"\n')
        f.write('config.JobType.psetName = "PSet.py"\n')
        f.write('config.JobType.scriptExe = "./DY_crab_script.sh" \n')
        f.write('config.JobType.inputFiles = ["../../scripts/haddnano.py","./DY_Template_postproc.py","./DY_Template_Module.py","./DY_keep_and_drop.txt","./DY_output_branch_selection.txt"] #hadd nano will not be needed once nano tools are in cmssw \n')
        ...
        f.write('config.Data.outLFNDirBase ="/store/user/sdeng/WZG_analysis/DY_test/' + sample_type + '/' + year + '"\n')
...
def submit(name,sample_type,year):
    ...
    if not os.path.exists(f'DY_crabcode_{year}/{abbre_name}_cfg.py'):
        print ("DY_crabcode for ",abbre_name," not existed, \033[31mskipping\033[0m")
        return True

    r=subprocess.run(args=f"crab submit -c DY_crabcode_{year}/{abbre_name}_cfg.py",shell=True,stdout=subprocess.PIPE,encoding='utf-8')
...
def kill(name,sample_type,year):
    ...
    if not os.path.exists(f'DY_crab{year}/crab_{abbre_name}'):
        print ("crab log for ",abbre_name," not existed, \033[31mskipping\033[0m \n")
        return True

    r=subprocess.run(args=f"crab kill -d DY_crab{year}/crab_{abbre_name}" ,shell=True,stdout=subprocess.PIPE,encoding='utf-8')
    print (r.stdout,'\n')

    shutil.rmtree(f'DY_crab{year}/crab_{abbre_name}')
    print (f'DY_crab{year}/crab_{abbre_name} has been removed')
...
def status(name,sample_type,year):
    ...
    r=subprocess.run(args=f"crab status -d DY_crab{year}/crab_{abbre_name}" ,shell=True,stdout=subprocess.PIPE,encoding='utf-8')
    print (r.stdout,'\n')
...
def hadd_help(name,sample_type,year):
    ...
    abbre_name = get_abbre(name,sample_type,year)
    store_path = '/eos/user/s/sdeng/WZG_analysis/DY_test/'
...
def report_lumi(name,sample_type,year):
    ...
    r=subprocess.run(args=f"crab report -d DY_crab{year}/crab_{abbre_name}" ,shell=True,stdout=subprocess.PIPE,encoding='utf-8')
    print (r.stdout,'\n')

    if not os.path.exists(f'DY_lumi_{year}'):
        os.mkdir(f'DY_lumi_{year}')
    
    shutil.copy(f'DY_crab{year}/crab_{abbre_name}/results/notFinishedLumis.json', f'DY_lumi_{year}/{abbre_name}.json')
...
def resubmit(name,sample_type,year):
    ...
    r = subprocess.run(args=f"crab resubmit -d DY_crab{year}/crab_{abbre_name}" ,shell=True,stdout=subprocess.PIPE,encoding='utf-8')
    print (r.stdout,"\n")
